...
def upgrade() -> None:
    ...
    # Check and create questtypeenum
    result = connection.execute(sa.text("SELECT 1 FROM pg_type WHERE typname = 'questtypeenum'")).fetchone()
    if not result:
        op.execute(sa.text("CREATE TYPE questtypeenum AS ENUM ('daily_login', 'feed_pet', 'earn_xp')"))
    
    # Check and create queststatusenum
    result = connection.execute(sa.text("SELECT 1 FROM pg_type WHERE typname = 'queststatusenum'")).fetchone()
    if not result:
