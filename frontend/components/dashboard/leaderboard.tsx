...
import { Trophy, Star, Medal, Flame, Award, Crown, Users, Calendar, History, Search, RefreshCw, Loader2 } from "lucide-react"
...
export function Leaderboard({ courseId, className }: LeaderboardProps) {
  ...
  const displayedOtherUsers = searchQuery.trim() ? searchResults.slice(3) : otherUsers;
  const displayedTopUsers = searchQuery.trim() ? searchResults.slice(0, 3) : topUsers;
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">
                {totalParticipants > 0 ? `${totalParticipants} participants` : 'Compete with others and climb the ranks'}
              </p>
            </div>
          </div>
      ...
      {!loading || data.topUsers.length > 0 ? (
        <div className="relative p-6 pb-16 bg-gradient-to-b from-background via-background to-muted/20">
            ...
            <div className="flex items-end justify-center mb-8 mt-4">
                ...
                >
                  <div className="flex flex-col items-center">
                    <Avatar className="h-16 w-16 border-4 border-primary/20 ring-2 ring-primary/10 mb-2 shadow-xl">
                      <AvatarImage src={displayedTopUsers[1].profile_image_url || undefined} />
                      <AvatarFallback className="bg-gradient-to-br from-blue-500 to-purple-600 text-white">
                        {`${displayedTopUsers[1].first_name[0] || displayedTopUsers[1].username[0]}${displayedTopUsers[1].last_name[0] || displayedTopUsers[1].username[1] || ''}`}
                      </AvatarFallback>
                    </Avatar>
                  ...
                  <div className="h-28 mt-2 bg-gradient-to-t from-card/80 to-card/50 dark:from-card to-background border border-border/50 backdrop-blur-sm rounded-t-lg flex items-center justify-center px-4 shadow-lg">
                    <div className="text-center">
                      <div className="font-medium text-2xl bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-purple-600 dark:text-primary/90">{displayedTopUsers[1].stats.exp_points}</div>
                ...
                >
                  ...
                  <div className="flex flex-col items-center">
                    <Avatar className="h-20 w-20 border-4 border-yellow-500/30 ring-4 ring-yellow-500/20 mb-2 shadow-xl">
                      <AvatarImage src={displayedTopUsers[0].profile_image_url || undefined} />
                      <AvatarFallback className="bg-gradient-to-br from-yellow-500 to-amber-600 text-white">
                        {`${displayedTopUsers[0].first_name[0] || displayedTopUsers[0].username[0]}${displayedTopUsers[0].last_name[0] || displayedTopUsers[0].username[1] || ''}`}
                      </AvatarFallback>
                    </Avatar>
                    <div className="text-center">
                  ...
                  <div className="h-36 mt-2 bg-gradient-to-t from-card/80 to-card/50 dark:from-card to-background border border-border/50 backdrop-blur-sm rounded-t-lg flex items-center justify-center px-4 relative overflow-hidden shadow-lg">
                    ...
                    <div className="text-center relative">
                      <div className="font-medium text-3xl bg-clip-text text-transparent bg-gradient-to-r from-yellow-600 to-amber-500 dark:text-primary/90">{displayedTopUsers[0].stats.exp_points}</div>
                      <div className="text-xs text-muted-foreground">XP POINTS</div>
                    </div>
                  </div>
                </motion.div>
              ...
              {displayedTopUsers[2] && (
                ...
                >
                  <div className="flex flex-col items-center">
                    <Avatar className="h-16 w-16 border-4 border-primary/20 ring-2 ring-primary/10 mb-2 shadow-xl">
                      <AvatarImage src={displayedTopUsers[2].profile_image_url || undefined} />
                      <AvatarFallback className="bg-gradient-to-br from-emerald-500 to-green-600 text-white">
                        {`${displayedTopUsers[2].first_name[0] || displayedTopUsers[2].username[0]}${displayedTopUsers[2].last_name[0] || displayedTopUsers[2].username[1] || ''}`}
                      </AvatarFallback>
                    </Avatar>
                    <div className="text-center">
                      <div className="font-semibold truncate max-w-[80px] sm:max-w-full">
                  ...
                  <div className="h-24 mt-2 bg-gradient-to-t from-card/80 to-card/50 dark:from-card to-background border border-border/50 backdrop-blur-sm rounded-t-lg flex items-center justify-center px-4 shadow-lg">
                    <div className="text-center">
                      <div className="font-medium text-2xl bg-clip-text text-transparent bg-gradient-to-r from-emerald-600 to-green-600 dark:text-primary/90">{displayedTopUsers[2].stats.exp_points}</div>
                      <div className="text-xs text-muted-foreground">XP POINTS</div>
                    </div>
                  </div>
                </motion.div>
          ...
          <div className="space-y-3">
            <AnimatePresence>
              {displayedOtherUsers.map((user, index) => renderUserCard(user, index))}
            </AnimatePresence>
          </div>
          ...
          {/* Empty state */}
          {!loading && displayedTopUsers.length === 0 && displayedOtherUsers.length === 0 && (
            <div className="text-center py-12">
              <Trophy className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
              <h3 className="text-lg font-medium mb-2">No participants yet</h3>
              <p className="text-muted-foreground">
                {searchQuery.trim() ? 'No players found matching your search.' : 'Be the first to join the leaderboard!'}
              </p>
            </div>
          )}
        </div>
